...
for epoch in range(training_epochs):
	...
	for i in range(numiters):
		if (i+1)*batch_size > train_size :
			batchX = dataX[train_size-batch_size : train_size]
			batchY = dataY[train_size-batch_size : train_size]
		else :
			batchX = dataX[i*batch_size : (i+1)*batch_size]
			batchY = dataY[i*batch_size : (i+1)*batch_size]
		# batchpoints = random.sample(traindata, batch_size)
		# batchX = [row[0] for row in batchpoints]
		# batchY = [row[1] for row in batchpoints]
	...
	if epoch%accuracy_iter==0 : 
		print("Test accuracy:", accuracy.eval(feed_dict={x: dataX, y_: dataY, pkeep1: 1.0}))
		save_path = saver.save(sess, "./model1.ckpt")
		print "Model saved in file %s" % save_path


acc_val = accuracy.eval(feed_dict={x: dataX, y_: dataY, pkeep1: 1.0})
